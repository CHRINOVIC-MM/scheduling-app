...
var toSoft = function(diff2) {
  ...
  let abbreviated = [];
  let englishValues = [];
  Object.keys(diff2).forEach((unit, i, units5) => {
...
units3.date = units3.day;
var startOf = (a, unit) => {
  let s = a.clone();
  unit = normalize2(unit);
  if (units3[unit]) {
    return units3[unit](s);
  }
  if (unit === "summer" || unit === "winter") {
    s = s.season(unit);
    return units3.season(s);
  }
  return s;
};
var endOf = (a, unit) => {
...
function useTimezoneSelect({
  ...
  const options2 = React6.useMemo(() => {
    return Object.entries(timezones2).map((zone) => {
      var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l;
      try {
        const now = src_default.now(zone[0]);
        ...
        switch (labelStyle) {
          ...
          case "altName":
            ...
            break;
          case "abbrev":
            label = `${prefix3} (${abbr.substring(0, maxAbbrLength)})`;
            break;
          default:
            label = `${prefix3}`;
        }
  ...
  const findFuzzyTz = (zone) => {
    let currentTime = src_default.now("GMT");
    try {
      currentTime = src_default.now(zone);
    } catch (err) {
      return;
    }
